...
import ru.itis.saetova.wizard.ui.HardcodedResourcesUIBuilder

class HardcodedResourcesStep(
    project: Project,
    model: CreateRuleWithParamsWizardModel,
    stepStateListener: (WizardStepState) -> Unit
) : BaseWizardStep<CreateRuleWithParamsWizardModel, HardcodedResourcesUIBuilder, RuleParamsStepState>(model, project, stepStateListener, STEP_TITLE) {

    companion object {
        const val STEP_TITLE = "Specify the options"
    }

    override fun getViewBuilder(): HardcodedResourcesUIBuilder = HardcodedResourcesUIBuilder(project)
}