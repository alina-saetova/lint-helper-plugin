...
import ru.itis.saetova.wizard.ui.AttributesInitUIBuilder

class AttributesInitStep(
    project: Project,
    model: CreateRuleWithParamsWizardModel,
    stepStateListener: (WizardStepState) -> Unit
) : BaseWizardStep<CreateRuleWithParamsWizardModel, AttributesInitUIBuilder, RuleParamsStepState>(model, project, stepStateListener, STEP_TITLE) {

    companion object {
        const val STEP_TITLE = "Specify the options"
    }

    override fun getViewBuilder(): AttributesInitUIBuilder = AttributesInitUIBuilder(project)
}