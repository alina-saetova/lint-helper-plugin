...
import ru.itis.saetova.wizard.ui.ReturnValueIsUsedUIBuilder

class ReturnValueIsUsedStep(
    project: Project,
    model: CreateRuleWithParamsWizardModel,
    stepStateListener: (WizardStepState) -> Unit
) : BaseWizardStep<CreateRuleWithParamsWizardModel, ReturnValueIsUsedUIBuilder, RuleParamsStepState>(model, project, stepStateListener, STEP_TITLE) {

    companion object {
        const val STEP_TITLE = "Specify the options"
    }

    override fun getViewBuilder(): ReturnValueIsUsedUIBuilder = ReturnValueIsUsedUIBuilder(project)
}