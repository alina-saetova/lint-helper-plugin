...
import ru.itis.saetova.wizard.ui.CheckObjectsInitUIBuilder

class CheckObjectsInitStep(
    project: Project,
    model: CreateRuleWithParamsWizardModel,
    stepStateListener: (WizardStepState) -> Unit
) : BaseWizardStep<CreateRuleWithParamsWizardModel, CheckObjectsInitUIBuilder, RuleParamsStepState>(model, project, stepStateListener, STEP_TITLE) {

    companion object {
        const val STEP_TITLE = "Specify the options"
    }

    override fun getViewBuilder(): CheckObjectsInitUIBuilder = CheckObjectsInitUIBuilder(project)
}