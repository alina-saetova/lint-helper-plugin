...
import ru.itis.saetova.wizard.ui.RequiresApiLevelUIBuilder

class RequiresApiLevelStep(
    project: Project,
    model: CreateRuleWithParamsWizardModel,
    stepStateListener: (WizardStepState) -> Unit
) : BaseWizardStep<CreateRuleWithParamsWizardModel, RequiresApiLevelUIBuilder, RuleParamsStepState>(model, project, stepStateListener, STEP_TITLE) {

    companion object {
        const val STEP_TITLE = "Specify the options"
    }

    override fun getViewBuilder(): RequiresApiLevelUIBuilder = RequiresApiLevelUIBuilder(project)
}