...
import ru.itis.saetova.wizard.ui.MethodParamsRuleUIBuilder

class MethodParamsRuleStep(
    project: Project,
    model: CreateRuleWithParamsWizardModel,
    stepStateListener: (WizardStepState) -> Unit
) : BaseWizardStep<CreateRuleWithParamsWizardModel, MethodParamsRuleUIBuilder, RuleParamsStepState>(model, project, stepStateListener, STEP_TITLE) {

    companion object {
        const val STEP_TITLE = "Specify the options"
    }

    override fun getViewBuilder(): MethodParamsRuleUIBuilder = MethodParamsRuleUIBuilder(project)
}