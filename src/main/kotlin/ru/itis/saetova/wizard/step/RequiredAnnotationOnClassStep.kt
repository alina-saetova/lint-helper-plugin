...
import ru.itis.saetova.wizard.ui.RequiredAnnotationOnClassUIBuilder

class RequiredAnnotationOnClassStep(
    project: Project,
    model: CreateRuleWithParamsWizardModel,
    stepStateListener: (WizardStepState) -> Unit
) : BaseWizardStep<CreateRuleWithParamsWizardModel, RequiredAnnotationOnClassUIBuilder, RuleParamsStepState>(model, project, stepStateListener, STEP_TITLE) {

    companion object {
        const val STEP_TITLE = "Specify the options"
    }

    override fun getViewBuilder(): RequiredAnnotationOnClassUIBuilder = RequiredAnnotationOnClassUIBuilder(project)
}