...
import ru.itis.saetova.wizard.ui.CheckChildrenViewsUIBuilder

class CheckChildrenViewsStep(
    project: Project,
    model: CreateRuleWithParamsWizardModel,
    stepStateListener: (WizardStepState) -> Unit
) : BaseWizardStep<CreateRuleWithParamsWizardModel, CheckChildrenViewsUIBuilder, RuleParamsStepState>(model, project, stepStateListener, STEP_TITLE) {

    companion object {
        const val STEP_TITLE = "Specify the options"
    }

    override fun getViewBuilder(): CheckChildrenViewsUIBuilder = CheckChildrenViewsUIBuilder(project)
}