...
import ru.itis.saetova.wizard.ui.RequiredAnnotationOnMethodUIBuilder

class RequiredAnnotationOnMethodStep(
    project: Project,
    model: CreateRuleWithParamsWizardModel,
    stepStateListener: (WizardStepState) -> Unit
) : BaseWizardStep<CreateRuleWithParamsWizardModel, RequiredAnnotationOnMethodUIBuilder, RuleParamsStepState>(model, project, stepStateListener, STEP_TITLE) {

    companion object {
        const val STEP_TITLE = "Specify the options"
    }

    override fun getViewBuilder(): RequiredAnnotationOnMethodUIBuilder = RequiredAnnotationOnMethodUIBuilder(project)
}